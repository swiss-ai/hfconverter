...
def split_qkv_gate(
    ...
    """Split a fused QKV weight that also carries the attention output gate.

    With ``attention_output_gate`` Megatron widens each KV group with one gate block per
    query head, placed between the query and key blocks::

        [query head rows..., gate head rows..., key head rows, value head rows]

    The gate blocks are returned in the same per-head order as the query blocks, so the
    flattened gate lines up channel-for-channel with the attention output it multiplies.
    For a weight the input shape is ``[(2*Q + 2*K)*D, hidden]``; a bias omits ``hidden``.
    """
    if num_q_heads <= 0 or num_kv_heads <= 0 or num_q_heads % num_kv_heads != 0:
        raise ValueError(
            f"split_qkv_gate: num_q_heads={num_q_heads} must be a positive multiple of "
            f"num_kv_heads={num_kv_heads}"
        )
    if qkv.dim() not in (1, 2):
        raise ValueError(
            f"split_qkv_gate: expected a 2D weight or 1D bias, got shape {tuple(qkv.shape)}"
        )
    query_heads_per_kv_group = num_q_heads // num_kv_heads
    total_head_blocks = 2 * num_q_heads + 2 * num_kv_heads
    blocks_per_kv_group = 2 * query_heads_per_kv_group + 2
    expected_first_dimension = total_head_blocks * head_dim
    if qkv.shape[0] != expected_first_dimension:
        raise ValueError(
            f"split_qkv_gate: dim 0 is {qkv.shape[0]}, expected (2*num_q_heads + "
            f"2*num_kv_heads) * head_dim = (2*{num_q_heads} + 2*{num_kv_heads}) * {head_dim} "
            f"= {expected_first_dimension}"
        )
    ...
    gate_indices = query_indices + query_heads_per_kv_group
    key_indices = torch.arange(
        2 * query_heads_per_kv_group, total_head_blocks, blocks_per_kv_group
    )
    value_indices = torch.arange(
        2 * query_heads_per_kv_group + 1, total_head_blocks, blocks_per_kv_group
    )

    query = head_blocks[query_indices].reshape(num_q_heads * head_dim, *remaining_shape)
    gate = head_blocks[gate_indices].reshape(num_q_heads * head_dim, *remaining_shape)
    key = head_blocks[key_indices].reshape(num_kv_heads * head_dim, *remaining_shape)
    value = head_blocks[value_indices].reshape(num_kv_heads * head_dim, *remaining_shape)
