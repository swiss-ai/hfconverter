...
class TestContractPlumbing:
    ...
    def test_removed_keel_fields_cannot_enable_a_legacy_residual_mode(self):
        ...
        assert not hasattr(inactive, "keel")
        assert not hasattr(inactive, "keel_alpha")

        with pytest.raises(ValueError, match="KEEL residual mode is not supported"):
            Apertus2Config(keel=True)
        with pytest.raises(ValueError, match="KEEL residual mode is not supported"):
            Apertus2Config(keel_alpha=3.0)
